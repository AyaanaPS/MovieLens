...
np.random.shuffle(Y)
Y, Y_out = Y[:int(.8 * len(Y)),:], Y[int(.8 * len(Y)):,:]
# Solution set for CS 155 Set 6, 2016/2017
# Authors: Fabian Boemer, Sid Murching, Suraj Nair
...
def train_model(M, N, K, eta, reg, Y, Y_out, eps=0.0001, max_epochs=300):
    """
    ...
    for epoch in range(max_epochs):
        ...
        E_out = get_err(U, V, Y_out, reg)
        print("Epoch %s, E_out (regularized MSE): %s"%(epoch + 1, E_out))
...
U,V,err = train_model(M,N,20,0.03,0.1,Y,Y_out)